...

{
    ...
    for(;;) {
        ...
        if(scd4x_read_measurement(&sensors_values) != ESP_OK) {
            ESP_LOGE("Carbon dioxide", "Sensors read measurement error!");
        }
        
        uint16_t co2 = sensors_values.co2;
        ESP_LOGI("Carbon dioxide", "CO2 = %d", co2);
        co2_value = (uint16_t)(co2);
    ...
    while(1)
    {
        if (connected)
        {
            ...
            if (co2_value != 0) 
            {
                ...
                esp_zb_zcl_status_t state_co2_value = esp_zb_zcl_set_attribute_val(SENSOR_ENDPOINT, ESP_ZB_ZCL_CLUSTER_ID_CARBON_DIOXIDE_MEASUREMENT, ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, ESP_ZB_ZCL_ATTR_CARBON_DIOXIDE_MEASUREMENT_MEASURED_VALUE_ID, (zb_uint8_t *)&co2_fixed, false);                
                /* Check for error */
                if(state_co2_value != ESP_ZB_ZCL_STATUS_SUCCESS)
                {
                    ESP_LOGE(TAG, "Setting CO2 attribute failed!");
                }

                /* CO2 Cluster must report it manually*/
                 reportAttribute(SENSOR_ENDPOINT, ESP_ZB_ZCL_CLUSTER_ID_CARBON_DIOXIDE_MEASUREMENT, ESP_ZB_ZCL_ATTR_CARBON_DIOXIDE_MEASUREMENT_MEASURED_VALUE_ID, (zb_uint8_t *)&co2_fixed, 2);
            }
        }
...
static void esp_zb_task(void *pvParameters)
{
    ...
    esp_zb_carbon_dioxide_measurement_cluster_add_attr(esp_zb_carbon_dioxide_measurement_cluster, ESP_ZB_ZCL_ATTR_CARBON_DIOXIDE_MEASUREMENT_MEASURED_VALUE_ID, &undefined_value);
    esp_zb_carbon_dioxide_measurement_cluster_add_attr(esp_zb_carbon_dioxide_measurement_cluster, ESP_ZB_ZCL_ATTR_CARBON_DIOXIDE_MEASUREMENT_MIN_MEASURED_VALUE_ID, &undefined_value);
    esp_zb_carbon_dioxide_measurement_cluster_add_attr(esp_zb_carbon_dioxide_measurement_cluster, ESP_ZB_ZCL_ATTR_CARBON_DIOXIDE_MEASUREMENT_MAX_MEASURED_VALUE_ID, &undefined_value); 
...
void app_main(void)
{
    ...
    if (!DEMO_MODE)
    {
        ...
        xTaskCreate(co2_task, "co2_task", 4096, NULL, 3, NULL);
    ...
    xTaskCreate(update_attribute, "Update_attribute_value", 4096, NULL, 4, NULL);
  ...
  zb_uint8_t endpoint, zb_uint16_t attr_id, zb_uint8_t *new_value)
{
  ...
  if (attr_id == ESP_ZB_ZCL_ATTR_CARBON_DIOXIDE_MEASUREMENT_MEASURED_VALUE_ID)
  {
